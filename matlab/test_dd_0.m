...
for subdiv_num = 1:length(subdivisions_vec)
    ...
    for alpha_num = 1:length(alpha_vec)
        ...
        errors_D1D0_method1(subdiv_num, alpha_num) = mean(abs((1 ./ face_areas) .* c2_method1).^err_order)^(1/err_order);
        errors_D2D1_method1(subdiv_num, alpha_num) = mean(abs((1 ./ volumes) .* c3_method1).^err_order)^(1/err_order);

        errors_D1D0_method2(subdiv_num, alpha_num) = mean(abs((1 ./ face_areas) .* c2_method2).^err_order)^(1/err_order);
        errors_D2D1_method2(subdiv_num, alpha_num) = mean(abs((1 ./ volumes) .* c3_method2).^err_order)^(1/err_order);
    end
end
...
% Test only method 2
matrices = {errors_D1D0_method2, errors_D2D1_method2};
titles = {'error D_1^\alpha D_0^\alpha', 'error D_2^\alpha D_1^\alpha'};
for i = 1:length(matrices)
    title_i = titles{i};
    errors = matrices{i};
